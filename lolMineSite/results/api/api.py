...
riotAPI = "RGAPI-e6509919-fb7b-4ee8-9ec1-7a3f565bb0af"
seasonID = "11"
endIndex = "5"
winnerEndIndex = "5"
ROLEFLAG = 0
...
def main(summonerName,role,roleSec):
    ...
    os.remove('results/api/lolLive.xlsx')
    writer = pd.ExcelWriter('results/api/lolLive.xlsx')
    df.to_excel(writer)
    writer.save()

    collected_data = pd.read_excel('results/api/lol.xlsx')
    collected_data = collected_data.append(df,ignore_index=True)
    collected_data = collected_data.drop_duplicates()
    print(collected_data)

    os.remove('results/api/lol.xlsx')
    collector = pd.ExcelWriter('results/api/lol.xlsx')
    collected_data.to_excel(collector)
    collector.save()
    return {'live':df,'collected':collected_data}
